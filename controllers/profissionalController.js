...
const deletarProfissional = async (req, res) => {
  try {
    ...
    return res.status(500).json({ mensagem: 'Erro ao deletar profissional.' });
  }
};
...
// Listar todos os profissionais (ou filtrar por CRM/nome)
const listarProfissionais = async (req, res) => {
  try {
    const { crm, nome } = req.query;
    let filtro = {};
    ...
    const profissionais = await Profissional.find(filtro).sort({ nome: 1 });
